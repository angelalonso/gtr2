...
Features:
- Preview race data before import
...
- Import vehicles from GTR2 install folder
- Manage unassigned vehicles and assign them to classes
"""
    ...
    def run(self):
        try:
            ...
            if not teams_dir.exists():
                self.error.emit(f"Teams directory not found: {teams_dir}")
                return
            
            # Find all .car and .CAR files recursively
            car_files = []
            for ext in ['*.car', '*.CAR']:
                car_files.extend(teams_dir.rglob(ext))
            
            if not car_files:
                self.error.emit(f"No .car files found in {teams_dir}")
                return
            ...
            for i, car_file in enumerate(car_files):
                ...
                self.progress.emit(i + 1, total_files, f"Scanning: {car_file.name}")
                ...
                try:
                    ...
                    # Also look for Description=... without quotes (some mods use this)
...
class VehicleClassesManager:
    ...
    def get_all_vehicles(self) -> Set[str]:
        """Get all vehicles from all classes"""
        all_vehicles = set()
    ...
    def add_vehicles_batch(self, class_name: str, vehicle_names: List[str]) -> int:
        """Add multiple vehicles to a class"""
        if class_name not in self.data:
            return 0
    ...
    def get_unassigned_vehicles(self, all_vehicles: Set[str]) -> List[str]:
        """Get vehicles that are not in any class"""
...
class VehicleClassesWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.import_cars_btn.setStyleSheet("background-color: #FF9800;")
        ...
        # Main content splitter - now with three panels
        ...
        self.add_to_class_btn.setStyleSheet("background-color: #4CAF50;")
    ...
    def select_gtr2_folder(self):
        ...
        folder = QFileDialog.getExistingDirectory(
            self, "Select GTR2 Installation Folder",
            str(Path.home())
        )
    ...
    def import_cars(self):
        """Import cars from GTR2 installation"""
    ...
    def add_selected_to_class(self):
        """Add selected unassigned vehicles to the currently selected class"""
        ...
        if reply == QMessageBox.Yes:
            ...
            if added > 0:
                self.refresh_unassigned_list()
                self.on_class_selected()  # Refresh the vehicles list
...
class RaceDataRecord:
    """Represents a single record from race data CSV"""
    vehicle: str
    timestamp: str
    track: str
    qual_ratio: float
    ...
    @classmethod
    def from_csv_row(cls, row: Dict[str, str]) -> Optional['RaceDataRecord']:
        """Create a RaceDataRecord from a CSV row dictionary"""
        try:
...
class DataImportWorker(QThread):
    ...
    def _read_data_file(self) -> List[RaceDataRecord]:
        """Read and parse the data file (CSV format)"""
        records = []
        ...
        with open(self.data_file_path, 'r', encoding='utf-8-sig') as f:
            ...
            for row_num, row in enumerate(reader, 2):
                ...
                record = RaceDataRecord.from_csv_row(row)
...
class DynAIDataManager(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.tab_widget.addTab(import_tab, "Race Data")
    ...
    def create_import_tab(self) -> QWidget:
        """Create the race data import tab"""
        tab = QWidget()
    ...
    def create_about_tab(self) -> QWidget:
        ...
        <ul>
            ...
            <li><b>Import vehicles from GTR2 installation</b> - scans all .car files</li>
            <li><b>Unassigned vehicles panel</b> shows vehicles not yet in any class</li>
            <li><b>Batch assign</b> multiple unassigned vehicles to a class</li>
    ...
    def browse_data_file(self):
        """Open file dialog to select data file (CSV)"""
    ...
    def on_classes_updated(self):
        ...
        self.statusBar().showMessage("Vehicle classes updated.", 3000)
        self.refresh_db_info()
