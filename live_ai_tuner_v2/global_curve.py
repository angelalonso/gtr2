...
class GlobalCurve:
    ...
    def predict_ratio(self, time: float, track_name: str) -> Optional[float]:
        ...
        params = self.track_params.get(track_name)
        if params:
            return self.ratio_from_time(time, params['a'], params['b'])
        
        # Bootstrap from available points
        points = self.points_by_track.get(track_name, [])
        if len(points) == 1:
            return self._bootstrap_one_point(time, track_name, points[0])
        elif len(points) >= 2:
            return self._bootstrap_two_points(time, points)
        
        # No data for this track, use global defaults with estimated M
        return self._bootstrap_no_data(time, track_name)
    ...
    def _bootstrap_two_points(self, time: float, points: List[Tuple[float, float]]) -> Optional[float]:
        ...
        a = (m1 - m2) / (inv_r1 - inv_r2)
        b = m1 - a * inv_r1
        
        # Validate parameters (b should be positive and less than lap times)
        if b <= 0 or b >= m1:
            logger.warning(f"Invalid parameters from two-point fit: a={a}, b={b}")
            return None
    ...
    def _fit_track_params(self, track_name: str):
        ...
        try:
            ...
            # Fallback to two-point method using first and last points
            if len(points) >= 2:
                r1, m1 = points[0]
                r2, m2 = points[-1]
                inv_r1 = 1.0 / r1
